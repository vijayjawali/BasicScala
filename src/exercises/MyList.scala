...
case object Empty extends MyList[Nothing]{
  ...
  def zipWith[B, C](list: MyList[B], zip: (Nothing, B) => C): MyList[C] = {
    if(!list.isEmpty) throw new RuntimeException("Lists did not have the same length")
    else Empty
  }
...
case class Cons[A](h: A, t: MyList[A]) extends MyList[A] {
  ...
  = new Cons(2, new Cons(4, new Cons(6, Emptu..map(n*2))))
...
object ListTest extends App{
  ...
  3. MyList:
      ...
      class EvenPredicate extends MyPredicate
  ...
  println(anotherListOfIntegers.printElements)
